...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // 4. טעינת טקסט "קצת עליי" מהשרת
    loadAboutText();
});

// פונקציית טעינת "קצת עליי" מעודכנת עם הצגת ספינר טעינה ומטמון (LocalStorage)
async function loadAboutText() {
    const aboutContainer = document.getElementById("about-text-container");
    if (!aboutContainer) return;

    const ABOUT_CACHE_KEY = "inbaliz_about_text";
    const cachedAbout = localStorage.getItem(ABOUT_CACHE_KEY);

    // אם יש מידע שמור במטמון, נציג אותו מיד בזמן שקט
    if (cachedAbout) {
        aboutContainer.innerHTML = cachedAbout;
    } else {
        // הצגת ספינר טעינה במידה ואין עדיין מידע שמור
        aboutContainer.innerHTML = `<div class="loading-spinner-container" style="text-align: center; padding: 20px;"><div class="loading-spinner"></div><p>טוען פרטים...</p></div>`;
    }

    try {
        const response = await fetch(`${GOOGLE_WEB_APP_URL}?type=about&t=${Date.now()}`);
        if (!response.ok) throw new Error("Could not fetch about text");

        const result = await response.json();
        if (result && result.text) {
            aboutContainer.innerHTML = result.text;
            // שמירת המידע המעודכן ב-LocalStorage
            localStorage.setItem(ABOUT_CACHE_KEY, result.text);
        }
    } catch (error) {
        console.error("Error loading about text:", error);
        // אם השרת נכשל ואין לנו כלום במטמון, נציג הודעת שגיאה או ברירת מחדל
        if (!cachedAbout) {
            aboutContainer.innerHTML = `<p style="text-align: center; color: #666;">לא ניתן לטעון את תוכן האודות כרגע.</p>`;
        }
    }
}
